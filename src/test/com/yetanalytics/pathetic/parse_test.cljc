(ns com.yetanalytics.pathetic.parse-test
...
(deftest parse-test-0
  (testing "Original JSONPath tests"
    (are [path v] (= v (parse/parse-first path))
...
(deftest parse-test-1
  (testing "JSONPath strings with dot notation"
    ...
    (are [path v] (= v (parse/parse-first path))
      ...
      "$..*"         ['.. '*])
    ;; Failures
    (are [path] (parse/is-parse-failure? (parse/parse-first path))
      ""
      ".store"
      "$."
      "$.."
...
(deftest parse-test-4
  (testing "JSONPath strings that have been joined with '|'"
    (are [paths v] (= v (parse/parse paths))
      ...
      "$.store[*] | $.results['b|ah']"
      [[["store"] '*] [["results"] ["b|ah"]]])
    (are [paths] (parse/is-parse-failure? (parse/parse paths))
      "$.store.book $.results.extension"
      "$.store.book | what the pineapple"
      "$['store|$['io']]")))

(deftest gen-tests
  (testing "Generative tests for parse namespace"
    (let [results
          (stest/check `[parse/is-parse-failure?
                         parse/test-strict-path
                         parse/path->string
                         parse/parse
                         parse/parse-first]
